...
gen-rfc-6890:
	...
	cargo run -- rfc 6890 >> $(RFC_6890_ENTRIES_RS).tmp
...
.PHONY: test
test:
	cargo run -- eval 'GetInterfaceIP "lo0" | FilterIPv4'
	cargo run -- eval 'GetAllInterfaces | FilterForwardable'
	cargo run -- eval 'GetAllInterfaces | FilterGlobal'
